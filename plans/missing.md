...
- [x] FEATURE: Header Generation V2 Anti-Hallucination System - Implemented 3-component header generation with Value Proposition Statement (40-word role-specific formula), Key Achievement Bullets (LLM-selected from master CV with JD matching), and Core Competencies (4 static sections per role, algorithmic selection with JD prioritization). Added `CoreCompetencyGeneratorV2`, role_skills_taxonomy.json, V2 dataclasses, and 50 comprehensive tests. V1 code fully removed 2025-12-31. ✅ **COMPLETED 2025-12-29**
- [x] BUG FIX: Per-Role Skills Hallucination - Fixed CVStitcher._compute_role_skills() to validate skills against candidate whitelist before inclusion. Orchestrator now passes whitelist parameter to stitcher (same pattern as HeaderGenerator). Prevents LLM from generating skills not in candidate's actual experience. 7 unit tests added. ✅ **COMPLETED 2025-12-31**
