...
if runner_bp:
    print("✅ Imported runner blueprint")
    ...
    print("❌ Could not import runner blueprint from any path")
    if _import_error:
...
if intel_bp:
    ...
    print("ℹ️ Intel dashboard blueprint not loaded (optional)")
        ...
        try:
            ...
            if is_dns_error:
                if attempt < max_retries - 1:
                    ...
                    print("   Hint: Run 'sudo dscacheutil -flushcache && sudo killall -HUP mDNSResponder' to clear DNS cache")
                    ...
                    print("   1. Flush DNS cache: sudo dscacheutil -flushcache && sudo killall -HUP mDNSResponder")
                    print("   2. Change DNS servers to 8.8.8.8 (Google DNS) in System Settings")
                    print("   3. Verify MongoDB connection: mongosh \"$MONGODB_URI\" --eval \"db.adminCommand('ping')\"")
                    print("   4. Restart Flask app after fixing DNS")
                    raise
            else:
...
def synthesize_persona(job_id: str):
    ...
    try:
        ...
        logger.error("Cannot connect to runner service for persona synthesis")
        return jsonify({
            "success": False,
...
def research_company(job_id: str):
    ...
    try:
        ...
        logger.error("Cannot connect to runner service for company research")
        return jsonify({
            "success": False,
...
def generate_cv(job_id: str):
    ...
    try:
        ...
        logger.error("Cannot connect to runner service for CV generation")
        return jsonify({
            "success": False,
...
def full_extraction(job_id: str):
    ...
    try:
        ...
        logger.error("Cannot connect to runner service for full extraction")
        return jsonify({
            "success": False,
