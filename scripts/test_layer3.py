"""
Quick test script for Layer 3: Company Researcher

Tests company research with web scraping + LLM summarization.
"""

from src.layer3.company_researcher import company_researcher_node
from src.common.state import JobState

# Sample job data (same as Layer 2 test for consistency)
...
EXPERIENCE: 6+ years managing and scaling a YouTube performance-based portfolio, with budgets of $20M+/year.
    ...
    # Optional fields (Layer 2 output would be filled in real workflow)
    "pain_points": None,
...
if __name__ == "__main__":
    print("Testing Layer 3: Company Researcher\n")

    # Run the node
    updates = company_researcher_node(sample_state)
    ...
    if updates.get("company_summary"):
        print("✅ SUCCESS! Company summary generated:")
        print(f"\n{updates['company_summary']}\n")

        if updates.get("company_url"):
            print(f"✅ Company URL found: {updates['company_url']}")
        else:
            print("⚠️  No company URL (used LLM fallback)")

    else:
        print("❌ FAILED: No company summary generated")
