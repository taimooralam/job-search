...
def main() -> int:
    parser = argparse.ArgumentParser(description="Smoke-run presentation_contract emphasis rules with visible heartbeats.")
    ...
    run_id = (
        f"preenrich:smoke:presentation_contract:emphasis_rules:"
    ...
    session = PreenrichTracingSession(
        ...
        metadata={
            ...
            "run_id": run_id,
            "worker_id": "smoke_emphasis_rules_subdocument",
    ...
    emphasis = result.stage_output.get("truth_constrained_emphasis_rules") or {}
    topic_coverage = {
        entry.get("topic_family"): entry.get("rule_count")
        for entry in (emphasis.get("topic_coverage") or [])
    ...
    session.complete(
        output={
            ...
            "stage_name": "presentation_contract",
            "emphasis_rules_status": emphasis.get("status"),
        ...
        json.dumps(
            {
                "summary": {
                    "status": result.stage_output.get("status"),
                    "emphasis_rules_status": emphasis.get("status"),
                    "global_rules_count": len(emphasis.get("global_rules") or []),
                    "allowed_if_evidenced_count": len(emphasis.get("allowed_if_evidenced") or []),
                    "downgrade_rules_count": len(emphasis.get("downgrade_rules") or []),
                    "omit_rules_count": len(emphasis.get("omit_rules") or []),
                    "forbidden_claim_patterns_count": len(emphasis.get("forbidden_claim_patterns") or []),
                    "credibility_ladder_rules_count": len(emphasis.get("credibility_ladder_rules") or []),
                    "mandatory_topic_coverage_count": sum(
                        1 for count in topic_coverage.values() if int(count or 0) >= 1
                    ),
                    "defaults_applied": emphasis.get("defaults_applied") or [],
                    "fail_open_reason": emphasis.get("fail_open_reason"),
                    "duration_s": duration_s,
                },
