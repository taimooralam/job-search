...
@pytest.fixture
def service(mock_db):
    """Create a JobSearchService instance with mocked database."""
...
class TestJobSearchService:
    ...
    def test_check_cache_found(self, service, mock_db):
        ...
        mock_db["job_search_cache"].find_one.return_value = mock_entry

        result = service._check_cache(cache_key)

        assert result == mock_entry
        mock_db["job_search_cache"].find_one.assert_called_once()

    def test_check_cache_not_found(self, service, mock_db):
        """Test cache lookup when entry doesn't exist."""
        mock_db["job_search_cache"].find_one.return_value = None
...
class TestJobSearchServiceActions:
    ...
    def test_get_job_found(self, service, mock_db):
        ...
        mock_db["job_search_index"].find_one.return_value = mock_job
    ...
    def test_get_job_not_found(self, service, mock_db):
        ...
        mock_db["job_search_index"].find_one.return_value = None

        result = service.get_job(str(ObjectId()))

        assert result is None

    def test_hide_job_success(self, service, mock_db):
        """Test hiding a job."""
        job_id = str(ObjectId())
        mock_db["job_search_index"].update_one.return_value = MagicMock(modified_count=1)
    ...
    def test_hide_job_not_found(self, service, mock_db):
        """Test hiding a non-existent job."""
        mock_db["job_search_index"].update_one.return_value = MagicMock(modified_count=0)
    ...
    def test_unhide_job_success(self, service, mock_db):
        """Test unhiding a job."""
        job_id = str(ObjectId())
        mock_db["job_search_index"].update_one.return_value = MagicMock(modified_count=1)
    ...
    def test_promote_job_success(self, service, mock_db):
        ...
        mock_db["job_search_index"].find_one.return_value = mock_job
        mock_db["level-2"].insert_one.return_value = MagicMock(inserted_id=level2_id)
        ...
        mock_db["level-2"].insert_one.assert_called_once()
        mock_db["job_search_index"].update_one.assert_called_once()

    def test_promote_job_already_promoted(self, service, mock_db):
    ...
    def test_promote_job_not_found(self, service, mock_db):
...
class TestJobSearchServiceCache:
    ...
    def test_clear_cache(self, service, mock_db):
        ...
        mock_db["job_search_cache"].delete_many.return_value = MagicMock(deleted_count=10)
    ...
    def test_get_cache_stats(self, service, mock_db):
        ...
        mock_db["job_search_cache"].count_documents.side_effect = [15, 10]  # total, active
    ...
    def test_get_index_stats(self, service, mock_db):
        """Test getting index statistics."""
        mock_db["job_search_index"].count_documents.side_effect = [100, 20, 5, 30]
...
class TestJobSearchServiceQuery:
    ...
    def test_query_index_basic(self, service, mock_db):
        ...
        mock_cursor.limit.return_value = mock_jobs
        mock_db["job_search_index"].find.return_value = mock_cursor
        mock_db["job_search_index"].count_documents.return_value = 1
        mock_db["job_search_index"].aggregate.return_value = []
    ...
    def test_query_index_with_filters(self, service, mock_db):
        ...
        mock_db["job_search_index"].find.return_value = mock_cursor
        mock_db["job_search_index"].count_documents.return_value = 0
        mock_db["job_search_index"].aggregate.return_value = []
        ...
        # Check that find was called with filters
        call_args = mock_db["job_search_index"].find.call_args
    ...
    def test_query_index_pagination(self, service, mock_db):
        ...
        mock_cursor.limit.return_value = []
        mock_db["job_search_index"].find.return_value = mock_cursor
        mock_db["job_search_index"].count_documents.return_value = 100
        mock_db["job_search_index"].aggregate.return_value = []
    ...
    def test_query_index_limit_capped(self, service, mock_db):
        ...
        mock_cursor.limit.assert_called_with(100)  # Should be capped at 100
...
class TestJobSearchServiceSearch:
    ...
    async def test_search_cache_hit(self, service, mock_db):
        ...
        mock_db["job_search_cache"].find_one.return_value = mock_cache_entry
        mock_db["job_search_index"].find.return_value = mock_jobs
    ...
    async def test_search_fresh(self, service, mock_db):
        ...
        with patch.object(service.sources["indeed"], "fetch_jobs", return_value=[mock_job]):
            # Mock upsert
            mock_db["job_search_index"].find_one_and_update.return_value = {
                "_id": ObjectId(),
                "title": "Engineer",
            }
            mock_db["job_search_index"].find.return_value = [
                {"_id": ObjectId(), "title": "Engineer", "company": "Tech"},
            ]
